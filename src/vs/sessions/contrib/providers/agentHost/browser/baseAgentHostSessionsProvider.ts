...
function toSessionGitHubPullRequestRefs(meta: SessionMeta | undefined, mapArtifactChat?: (chat: string) => URI | undefined): readonly IGitHubPullRequestRef[] {
	...
	const { pullRequestUrls, pullRequestChats, pullRequestTitles } = partitionSessionArtifacts(meta, mapArtifactChat);
...
function toGitHubInfo(meta: SessionMeta | undefined, mapArtifactChat?: (chat: string) => URI | undefined): IGitHubInfo | undefined {
	...
	const { issueUrls, issueTitles } = partitionSessionArtifacts(meta, mapArtifactChat);
...
export class AgentHostSessionAdapter extends Disposable implements ISession {
	...
	constructor(
		...
		this.artifacts = derivedOpts<readonly ISessionArtifact[]>({ owner: this, equalsFn: structuralEquals }, reader => {
			...
			return partitionSessionArtifacts(meta, chat => this._mapArtifactChat(chat)).entries.map(entry => entry.artifact);
		});
